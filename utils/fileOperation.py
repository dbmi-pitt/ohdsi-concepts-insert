...
# return dict {vocab_id|concept_code: concept_id}
def readConceptCache(cache_path):
    ...
    if os.path.isfile(cache_path):
        with open(cache_path) as f:
            ...
            for line in lines:
                if '|' in line:
                    [vocab_id, concept_code, concept_id] = line.strip().split('|')
                    cacheDict[vocab_id + '|' + concept_code] = concept_id
...
# input: cache file path
# input: dict {vocab_id|concept_code: concept_id}
...
# read local cache concept mapping file
# input: cache file path
# return dict {vocab_id|concept_name: concept_id}
def readVocabCache(cache_path):
    cacheDict = {}
    if os.path.isfile(cache_path):
        with open(cache_path) as f:
            lines = f.readlines()
...
# update local cache concept mapping
# input: cache file path
# input: dict {vocab_id|concept_name: concept_id}
def writeVocabCache(cache_path, cacheDict):
    with open(cache_path, 'w') as f:
        for cpt_key, concept_id in sorted(cacheDict.iteritems()):
            f.write(cpt_key+'|'+str(concept_id)+'\n')       
